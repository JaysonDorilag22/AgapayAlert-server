...
async function handleDescriptionInput(psid, text, session) {
  if (!text || text.trim().length < 10) {
    const helpText = "Please provide more details about what happened. Include when the person was last seen, what they were wearing, and any circumstances about their disappearance.\n\nExample: \"Juan was last seen yesterday around 3 PM at SM City. He was wearing a red shirt and black pants. He went to meet with friends and didn't return home.\"";
    
    return await sendResponse(psid, { text: helpText });
  }
  
  const trimmedDescription = text.trim();
  console.log(`Saving description (${trimmedDescription.length} chars): "${trimmedDescription.substring(0, 50)}..."`);
  
  try {
    // Update session with description - save to BOTH fields to ensure it's found
    await MessengerReportSession.findOneAndUpdate(
      { psid },
      { 
        'data.messengerDescription': trimmedDescription,
    ...
    await sendResponse(psid, { 
      text: "Thank you for the description. Now, please provide the last known location with as much detail as possible. Example: \"123 Main Street, Barangay San Jose, Quezon City\"" 
    });
  } catch (error) {
    console.error("Error saving description:", error);
    await sendResponse(psid, { 
      text: "We encountered an error saving your description. Please try again." 
    });
  }
}
...
// In handleDescriptionInput - Make sure we're saving to the right field
async function handleDescriptionInput(psid, text, session) {
  if (!text || text.trim().length < 10) {
    return await sendResponse(psid, { 
  ...
  const trimmedDescription = text.trim();
  console.log(`🔸 SAVING DESCRIPTION for ${psid}: "${trimmedDescription.substring(0, 50)}${trimmedDescription.length > 50 ? '...' : ''}"`);
  
  // Update session with description - Save EXPLICITLY to the messengerDescription field
  try {
    const updatedSession = await MessengerReportSession.findOneAndUpdate(
      { psid },
      { 
        'data.messengerDescription': trimmedDescription
      },
    ...
    console.log(`🔹 DESCRIPTION SAVED: ${updatedSession.data.messengerDescription ? 'YES' : 'NO'}`);
    
    // Set the step to next only after successfully saving description
    await MessengerReportSession.findOneAndUpdate(
      { psid },
      { currentStep: 'LOCATION' },
      { new: true }
    ...
    await sendResponse(psid, { 
      text: "Thank you for the description. Now, please provide the last known location with as much detail as possible ex: street name, barangay, city:" 
    });
  } catch (error) {
    console.error("❌ ERROR saving description to session:", error);
    await sendResponse(psid, { 
      text: "We encountered an error saving your description. Please try again." 
    });
  }
...
async function submitReport(psid) {
  try {
    ...
    // Extract description (check multiple possible fields)
    let description = '';
    let isAutoGenerated = false;
    
    if (reportData.messengerDescription && reportData.messengerDescription.trim().length > 0) {
      description = reportData.messengerDescription.trim();
      console.log(`Found description in messengerDescription field: ${description.substring(0, 50)}...`);
    } else if (reportData.description && reportData.description.trim().length > 0) {
      description = reportData.description.trim();
      console.log(`Found description in description field: ${description.substring(0, 50)}...`);
    } else {
      // Generate a basic description if none provided
      const firstName = reportData.personInvolved?.firstName || "Person";
      const lastName = reportData.personInvolved?.lastName || "";
      const age = reportData.personInvolved?.age || "unknown age";
      const type = reportData.type || "Missing";
      const city = reportData.location?.address?.city || "unknown location";
      
      description = `${firstName} ${lastName}, ${age} years old, was reported ${type.toLowerCase()} in ${city}. This report was submitted through Facebook Messenger. ${description ? description : ""}`;
      isAutoGenerated = true;
      console.log(`Generated description: ${description}`);
    }
    
    // Get coordinates and location data
    const location = {
      address: {
    ...
    // Try to get coordinates from session or geocoding
    ...
    if (!coordinates || !Array.isArray(coordinates) || coordinates.length !== 2) {
      ...
      if (geoData.success) {
        ...
        coordinates = [121.0244, 14.5547]; // Default to Manila
      }
    }
    ...
    if (!user) {
      try {
        console.log("Creating temporary user for PSID:", psid);
    ...
    // Find police station - using auto-assign
    const assignedStation = await findPoliceStation(null, coordinates, true);
    
    if (!assignedStation) {
      return await sendResponse(psid, { 
        text: "We couldn't find a police station to assign. Please try again later."
      });
    }
    
    console.log("Assigned station:", assignedStation._id);
    
    // Create report with the correct enum value
    const report = new Report({
      ...
      messengerDescription: description, // Setting description explicitly
      validCredential: reportData.credential || "Messenger report",
      stationAssignmentType: "automatic", // FIXED: Using correct enum value
    ...
    console.log("About to save report with data:", {
      ...
      description: `${description.substring(0, 30)}... (${description.length} chars)`,
      stationAssignmentType: report.stationAssignmentType // Using correct enum value
    });
    ...
    try {
      ...
      // Add description info to confirmation
      const descriptionNote = isAutoGenerated
        ? "\n\nA basic description was automatically generated."
      ...
      if (saveError.name === 'ValidationError') {
        ...
        await sendResponse(psid, { 
          text: "We encountered validation errors. Please try again or use the AgapayAlert app." 
        });
      } else {
        await sendResponse(psid, { 
          text: "We encountered an error saving your report. Please try again or use the AgapayAlert app." 
        });
      }
    }
  } catch (error) {
