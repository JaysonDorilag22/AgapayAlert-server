...
async function submitReport(psid) {
  try {
    ...
    // EXPLICITLY check for description in multiple possible paths
    let description = '';
    let isAutoGenerated = false;
    
    if (reportData.messengerDescription && reportData.messengerDescription.length > 0) {
      description = reportData.messengerDescription;
      console.log(`📝 FOUND DESCRIPTION AT: data.messengerDescription (${description.length} chars)`);
    } else if (reportData.description && reportData.description.length > 0) {
      description = reportData.description;
      console.log(`📝 FOUND DESCRIPTION AT: data.description (${description.length} chars)`);
    } else {
      console.log("📝 NO DESCRIPTION FOUND IN SESSION DATA - GENERATING DEFAULT");
      
      // Auto-generate a description using available information
      ...
      const streetAddress = reportData.location?.address?.streetAddress || "";
      
      // Build a comprehensive description using available details
      description = `${firstName} ${lastName}, ${age} years old, was reported ${type.toLowerCase()} in ${city}${streetAddress ? ` near ${streetAddress}` : ''}. This report was submitted through Facebook Messenger. The reporter provided a photo but no additional details about the circumstances.`;
    ...
    if (!coordinates || !Array.isArray(coordinates) || coordinates.length !== 2) {
      ...
      if (geoData && geoData.success) {
        ...
        // Default to Metro Manila if geocoding fails
    ...
    // Find police station - always use auto-assign for messenger reports
    ...
    // Calculate approximate last seen date and time from description if possible
    const lastSeenDate = new Date(); // Default to now
    let lastSeenTime = new Date().toTimeString().substring(0, 5); // Default to now (HH:MM)
    ...
    const report = new Report({
      ...
      messengerDescription: description, // Use messengerDescription field in the Report model
      ...
      stationAssignmentType: "AUTO_NEARBY", // Use valid enum value from your schema
      messengerData: {
        psid: psid,
    ...
    console.log("About to save report with data:", {
      ...
      description: description ? `${description.substring(0, 50)}${description.length > 50 ? '...' : ''}` : 'NONE',
      descriptionLength: description ? description.length : 0,
      stationAssignmentType: report.stationAssignmentType
    });
    ...
    try {
      ...
      // Add explicit description info to confirmation message
      const descriptionNote = isAutoGenerated ? 
        "\n\nA basic description was automatically generated. You can add more details later in the app." : 
        description && description.length > 0 ? 
          `\n\nYour description has been saved (${description.length} characters).` : 
          "\n\nNo description was provided. You can add details later in the app.";
      ...
      // Try to send an image confirmation too
      try {
        await sendResponse(psid, {
          attachment: {
            type: "image",
            payload: {
              url: report.personInvolved.mostRecentPhoto.url
            }
          }
        });
      } catch (imgError) {
        console.error("Error sending image confirmation:", imgError);
      ...
      if (saveError.name === 'ValidationError') {
        ...
        // Show specific error fields to the user
